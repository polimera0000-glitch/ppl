...
const RegisterScreen = () => {
  ...
  const [agreeBoth, setAgreeBoth] = useState(false); // ✅ must be checked
  ...
  const handleRegister = async (e) => {
    ...
    try {
      const payload = {
        ...
        role: selectedRole,
        // ✅ pass agreement flags; backend will stamp timestamps
        agree_tnc: true,
        agree_privacy: true,
      };
      ...
      <div className="mx-auto w-full max-w-md sm:max-w-lg bg-surface/80 backdrop-blur-xl p-4 sm:p-6 md:p-7 rounded-2xl border border-border shadow-card">
        <div className="flex items-center justify-between mb-5 sm:mb-6">
          <div className="flex items-center gap-3">
            ...
            <h1 className="text-primary-text text-xl sm:text-2xl font-bold">
              Register
            </h1>
          </div>
          {selectedRole && (
        ...
        >
          ...
          {/* ✅ Mandatory Terms & Privacy checkbox (replaces any Remember me) */}
          <div className="mt-2">
            <label className="flex items-start gap-2 cursor-pointer select-none">
              <input
                ...
                checked={agreeBoth}
                onChange={(e) => {
                  setAgreeBoth(e.target.checked);
                  if (errors.agreeBoth)
                    setErrors((prev) => ({ ...prev, agreeBoth: null }));
                }}
            ...
            {errors.agreeBoth && (
              <p className="mt-1 text-xs text-red-500">{errors.agreeBoth}</p>
            )}
          </div>
        ...
        <div className="flex items-center my-5 sm:my-6">
          ...
          <span className="px-3 text-secondary-text text-xs sm:text-sm">
            OR
          </span>
          <div className="flex-1 h-px bg-border" />
        </div>

...
const ProfileScreen = () => {
  ...
  // NEW: refs for added fields
  const genderRef = useRef(null);          // select
  const eduTypeRef = useRef(null);         // select
  const workExpRef = useRef(null);         // number

  const [skills, setSkills] = useState([]);
  const [eduTypeState, setEduTypeState] = useState("undergraduate"); // local for conditional UI (kept in sync with ref)
  const isLoggedIn = useMemo(() => !!authService.getToken(), []);
  ...
  const applyUserToForm = (u) => {
    if (nameRef.current) nameRef.current.value = (u?.name ?? "").toString();
    if (collegeRef.current) collegeRef.current.value = (u?.college ?? "").toString();
    if (branchRef.current) branchRef.current.value = (u?.branch ?? "").toString();
    if (yearRef.current) yearRef.current.value = (u?.year ?? "").toString();
    if (phoneRef.current) phoneRef.current.value = (u?.phone ?? "").toString();
    if (orgRef.current) orgRef.current.value = (u?.org ?? "").toString();
    if (countryRef.current) countryRef.current.value = (u?.country ?? "").toString();

    // NEW: set added fields
    if (genderRef.current) genderRef.current.value = (u?.gender ?? "prefer_not_to_say");
    const edu = (u?.edu_type ?? "undergraduate");
    if (eduTypeRef.current) eduTypeRef.current.value = edu;
    setEduTypeState(edu);
    if (workExpRef.current) workExpRef.current.value =
      u?.work_experience_years != null ? String(u.work_experience_years) : "";

    // skills can be array or comma-separated string
    const raw = u?.skills;
    let next = [];
    if (Array.isArray(raw)) next = raw.map(String).filter(Boolean);
    else if (typeof raw === "string") next = raw.split(",").map((s) => s.trim()).filter(Boolean);
    setSkills(next);
  };
  ...
  const saveProfile = async () => {
    try {
      ...
      const gender = genderRef.current?.value ?? "prefer_not_to_say";
      const edu_type = eduTypeRef.current?.value ?? "undergraduate";
      const work_experience_years_str = (workExpRef.current?.value ?? "").toString().trim();
      const work_experience_years =
        work_experience_years_str.length > 0 ? Number.parseInt(work_experience_years_str, 10) : undefined;
      ...
      // NEW validation: work exp only when graduate
      if (edu_type === "graduate") {
        if (work_experience_years == null || Number.isNaN(work_experience_years)) {
          alert("Please enter valid work experience (years).");
          setSaving(false);
          return;
        }
        if (work_experience_years < 0 || work_experience_years > 60) {
          alert("Work experience must be between 0 and 60 years.");
          setSaving(false);
          return;
        }
      }
      ...
      const payload = {
        ...
        gender,
        edu_type,
        work_experience_years: edu_type === "graduate" ? work_experience_years ?? 0 : 0,
      };
            ...
            {loading ? (
              ...
              <div className="space-y-5">
                ...
                {editing ? (
                  ...
                  <LabelValue label="Gender" icon={UserIcon}>
                ...
                {editing ? (
                  <>
                    ...
                    {eduTypeState === "graduate" && (
                      <TextField
                        label="Work Experience (years)"
                        hint="e.g., 2"
                        type="number"
                        defaultValue={
                          user?.work_experience_years != null ? String(user.work_experience_years) : ""
                        }
                        ref={workExpRef}
                        icon={Briefcase}
                        min={0}
                  ...
                  <>
                    <LabelValue label="Type" icon={GraduationCap}>
                      {user?.edu_type ? user.edu_type : "—"}
                    </LabelValue>
                    {user?.edu_type === "graduate" && (
                      <LabelValue label="Work Experience (years)" icon={Briefcase}>
                        {user?.work_experience_years ?? 0}
                      </LabelValue>
                    )}
                  </>
                )}
                ...
                {/* Read-only consent timestamps if present */}
                <LabelValue label="Agreed T&C">
                  {user?.agreed_tnc_at ? new Date(user.agreed_tnc_at).toLocaleString() : "—"}
                </LabelValue>
                <LabelValue label="Agreed Privacy">
                  {user?.agreed_privacy_at ? new Date(user.agreed_privacy_at).toLocaleString() : "—"}
                </LabelValue>

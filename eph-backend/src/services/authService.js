...
const EmailVerificationToken =
  models.EmailVerificationToken || models.EmailVerification || null; // support either name
...
// --- Token helpers ---
function generateRandomToken(bytes = 24) {
  return crypto.randomBytes(bytes).toString('hex'); // hex string length = bytes*2
}
function randomToken(len = 64) {
  // produce exactly `len` hex chars
...
async function markRecordUsed(rec) {
  ...
  if (typeof rec.update === 'function') {
    try {
      ...
      if ('used' in rec) updateObj.used = true;
      if ('is_used' in rec) updateObj.is_used = true;
      if ('used_at' in rec) updateObj.used_at = new Date();
      if (Object.keys(updateObj).length > 0) {
        return await rec.update(updateObj);
      }
      if ('status' in rec) return await rec.update({ status: 'used' });
      return await rec.update({ used: true }).catch(() => rec); // last resort
    } catch (_err) {
      return rec;
    }
  }
...
const authService = {
  // ------------------ JWT (access) ------------------
  generateToken: (user) => {
    if (!JWT_SECRET) throw new Error('JWT secret is not configured. Set JWT_SECRET in environment or config.');
    if (!user || !user.id) throw new Error('User object with "id" is required to generate token.');
  ...
  // ------------------ JWT (refresh) ------------------
  generateRefreshToken: (user) => {
    if (!JWT_REFRESH_SECRET) throw new Error('JWT refresh secret is not configured. Set JWT_REFRESH_SECRET.');
    if (!user || !user.id) throw new Error('User object with "id" is required to generate refresh token.');

    const payload = { sub: user.id, type: 'refresh' };
    return jwt.sign(payload, JWT_REFRESH_SECRET, { expiresIn: JWT_REFRESH_EXPIRES_IN, issuer: JWT_ISSUER });
  },
  ...
  // ------------------ Email verification ------------------
  async createEmailVerificationToken(userId, meta = {}) {
    if (!EmailVerificationToken) {
      throw new Error('EmailVerificationToken model not available. Ensure models.EmailVerificationToken is exported.');
    }
    const ttlSec = (config?.auth?.emailVerificationExpirySeconds) || 60 * 60 * 24; // 24h default
    const token = randomToken(64);

    const ip = typeof meta.ipAddress === 'string' ? meta.ipAddress : (meta.ipAddress ? String(meta.ipAddress) : null);
    const ua = typeof meta.userAgent === 'string' ? meta.userAgent : (meta.userAgent ? String(meta.userAgent) : null);

    const rec = await EmailVerificationToken.create({
      user_id: userId,
      token,
  ...
  async validateEmailVerificationToken(token) {
    ...
    const rec = await EmailVerificationToken.findOne({ where: { token } });
    if (!rec) return null;

    // used?
    if (('used' in rec && rec.used) || ('is_used' in rec && rec.is_used) || ('used_at' in rec && rec.used_at)) {
      return null;
    }
  ...
  // ------------------ Password reset ------------------
  // createPasswordResetToken: opts { expiresInSeconds, ipAddress, userAgent }
  createPasswordResetToken: async (userId, opts = {}) => {
    const expiresInSeconds = opts.expiresInSeconds || (config.auth && config.auth.passwordResetExpirySeconds) || 3600;
    const token = generateRandomToken(28);
    const expiresAt = new Date(Date.now() + expiresInSeconds * 1000);
    ...
    if (typeof PasswordReset.createForUser === 'function') {
      ...
      const payload = { token, user_id: userId, expires_at: expiresAt, used: false };
      const attrs = PasswordReset.rawAttributes || {};
      if ('ip_address' in attrs) payload.ip_address = ip;
      if ('user_agent' in attrs) payload.user_agent = ua;
      rec = await PasswordReset.create(payload);
    }
    ...
    try {
      return {
        id: (rec && (rec.id || (typeof rec.get === 'function' && rec.get('id')))) || null,
        token: (rec && (rec.token || (typeof rec.get === 'function' && rec.get('token')))) || token,
        expires_at: (rec && (rec.expires_at || rec.expiresAt)) || expiresAt,
      };
    } catch (_e) {
      return { id: null, token, expires_at: expiresAt };
    }
  ...
  validatePasswordResetToken: async (token) => {
    ...
    if (('used' in rec && rec.used) || ('is_used' in rec && rec.is_used)) return null;
  ...
  createMagicToken: async ({ userId, purpose = 'magic', expiresInSeconds = 600 } = {}) => {
    ...
    if (MagicToken) {
      if (typeof MagicToken.create === 'function') {
        return MagicToken.create({ token, user_id: userId, purpose, expires_at: expiresAt, used: false });
      }
      if (typeof MagicToken.createForUser === 'function') {
        return MagicToken.createForUser(userId, { token, purpose, expires_at: expiresAt });
      }
    ...
    if (PasswordReset) {
      const attrs = PasswordReset.rawAttributes || {};
      const payload = { token, user_id: userId, expires_at: expiresAt, used: false };
      if ('purpose' in attrs) payload.purpose = purpose;
      return PasswordReset.create(payload);
    }
  ...
  validateMagicToken: async (token) => {
    if (MagicToken) {
      if (typeof MagicToken.findValidToken === 'function') return MagicToken.findValidToken(token);
      const rec = await MagicToken.findOne({ where: { token } });
      if (!rec) return null;
      if (('used' in rec && rec.used) || ('is_used' in rec && rec.is_used)) return null;
      const expiresAt = rec.expires_at || rec.expiresAt || rec.expires;
    ...
    if (PasswordReset) {
      ...
      if (!rec) return null;
      const attrs = PasswordReset.rawAttributes || {};
      if ('purpose' in attrs && rec.purpose && !['magic', 'admin_magic'].includes(rec.purpose)) return null;
      if (('used' in rec && rec.used) || ('is_used' in rec && rec.is_used)) return null;
      const expiresAt = rec.expires_at || rec.expiresAt || rec.expires;
      if (expiresAt && new Date(expiresAt) < new Date()) return null;
      return rec;
